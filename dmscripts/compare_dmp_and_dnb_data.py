...
class DNBDirectAPIClient(object):
    ...
    def __init__(self, username, password, stage='production'):
        ...
        self.max_dnb_api_calls_per_method = 10000
    ...
    def fetch_and_compare_frameworks_suppliers_data(
        ...
        for framework in frameworks:
            ...
            for dmp_supplier in dmp_response:
                duns_number = dmp_supplier.get('declaration', {}).get('supplierDunsNumber') if from_declaration else dmp_supplier.get('dunsNumber')
                # don't compare if DMp has no DUNS for a supplier
                if duns_number is None:
                    print('NO DUNS NUMBER FOR', self.extract_dmp_supplier_data(dmp_supplier, from_declaration=from_declaration))  # TODO: raise/log error?
                    continue
